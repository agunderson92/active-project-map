...
"Active" means `Project_Stage__c` is anything other than **Archive**. Archived
projects are hidden by default but can be shown by checking **Archive** in the
stage filter (check only Archive to see archived projects exclusively).
...
- **Data** comes from two GraphQL queries (no server code):
  1. All projects + related users + parent Opportunity `Amount` + Account
     billing address (Archive is filtered client-side, not in the query).
...
- **Filter bar** above the map — filter by **stage** (multi-select, including
  Archive), **developer**, and **project start date range**. Filtering is
  client-side over the loaded set, so it's instant and shows a live count.
